...
export const Reservation = () => {
    ...
    const [currentSelectedServices, setCurrentSelectedServices] = useState(selectedServices);
    const [totalPrice, setTotalPrice] = useState(0);

    useEffect(() => {
        searchHotelRequest(rooms.idHotel)
            .then((response) => {
                if (response.data && response.data.hotelsFound && response.data.hotelsFound.length > 0) {
    ...
    useEffect(() => {
        const total = currentSelectedServices.reduce((sum, service) => sum + service.price, 0);
